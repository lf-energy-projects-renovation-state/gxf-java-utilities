...
class MessageSigningInterceptorIT {
    ...
    fun `can sign ByteArray message with interceptor`() {
        val producer = createByteArrayKafkaProducer(embeddedKafkaBroker, producerPropertiesForByteArrayRecords)
    ...
    fun `can sign Avro message with interceptor`() {
        val producer = createAvroKafkaProducer(embeddedKafkaBroker, producerPropertiesForAvroRecords)
        ...
        assertThat(signatureHeader).isNotNull
        assertThat(signatureHeader.value()).isNotEmpty()
        assertThat(messageSigner.verifyUsingHeader(receivedRecord)).isTrue()
    }
}
